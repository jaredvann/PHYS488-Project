...
class TestAttenuator {
	...
	public static void main(String[] args) {
		...
		energyLoss = iron.getEnergyLoss(Helpers.MASS_MUON, 30.0);
		screen.println(30 + "\t\t\t" + 81.565 + "\t\t\t" + df.format(energyLoss));

		energyLoss = iron.getEnergyLoss(Helpers.MASS_MUON, 300.0);
		screen.println(300 + "\t\t\t" + 11.599 + "\t\t\t" + df.format(energyLoss));

		energyLoss = iron.getEnergyLoss(Helpers.MASS_MUON, 3000.0);
		screen.println(3000 + "\t\t\t" + 15.388 + "\t\t\t" + df.format(energyLoss));

		energyLoss = iron.getEnergyLoss(Helpers.MASS_MUON, 10000.0);
		screen.println(10000 + "\t\t\t" + 18.078 + "\t\t\t" + df.format(energyLoss));

		energyLoss = iron.getEnergyLoss(Helpers.MASS_MUON, 30000.0);
		screen.println(30000 + "\t\t\t" + 20.547 + "\t\t\t" + df.format(energyLoss));

		energyLoss = iron.getEnergyLoss(Helpers.MASS_MUON, 100000.0);
		...
		theta0 = iron.getMCSTheta0(Helpers.MASS_MUON, 500.0);
		screen.println(500 + "\t\t\t0.020\t\t\t" + df.format(theta0));

		theta0 = iron.getMCSTheta0(Helpers.MASS_MUON, 1000.0);
		screen.println(1000 + "\t\t\t9.98E-03\t\t" + sf.format(theta0));

		theta0 = iron.getMCSTheta0(Helpers.MASS_MUON, 3000.0);
